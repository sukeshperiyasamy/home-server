...
* [x] **Phase 6A/B/C: Core Services (Vaultwarden & Tailscale HTTPS)** — Deployed Vaultwarden (v1.33.2-alpine) via Docker Compose; persistent data at `/mnt/storage/app-data/vaultwarden`; bound to Tailscale IP; secured with Tailscale Serve private HTTPS (`https://pi-server.tail1040b5.ts.net`).
* [x] **Phase 6D: File Management Dashboard (File Browser)** — Deployed File Browser (v2.63.23) via Docker Compose; persistent data at `/mnt/storage/app-data/filebrowser`; bound to Tailscale IP (`100.96.171.29:8082`); secured with Tailscale Serve dedicated HTTPS (`https://pi-server.tail1040b5.ts.net:8443`); strictly isolated from `app-data` and `backups`.
